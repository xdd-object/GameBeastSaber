  ...
  <div class="app-container">
    <el-card class="filter-container" shadow="never">
      ...
      <div style="margin-top: 15px">
        <el-form :inline="true" :model="listQuery" size="small" label-width="140px">
          ...
          <el-form-item label="for用户ID：">
            <el-input v-model="listQuery.forMemberId" class="input-width" placeholder="请输入for用户ID"></el-input>
          </el-form-item>
          <el-form-item label="订单ID：">
            <el-input v-model="listQuery.orderId" class="input-width" placeholder="请输入订单ID"></el-input>
          </el-form-item>
          <el-form-item label="追溯编号：">
            <el-input v-model="listQuery.initiation" class="input-width" placeholder="请输入追溯编号"></el-input>
          </el-form-item>
          <el-form-item label="状态：">
            <el-select clearable v-model="listQuery.status">
              <el-option label="完成" value="1"></el-option>
              <el-option label="待付款" value="2"></el-option>
              <el-option label="持有中" value="3"></el-option>
              <el-option label="交割" value="4"></el-option>
              <el-option label="转售" value="5"></el-option>
            </el-select>
          </el-form-item>
    ...
    <div class="table-container">
      <el-table ref="homeAdvertiseTable"
        ...
        <el-table-column label="收益天数"  align="center">
         ...
         <el-table-column label="状态" align="center">
          <template slot-scope="scope">
            {{scope.row.status == 1 ? '已完成' : scope.row.status == 2 ? '交易中' :  scope.row.status == 3 ? '持有中' :  scope.row.status == 4 ? '交割中' :  scope.row.status == 5 ? '待抢购' : '申诉中'}}</template>
        </el-table-column>
        ...
        <el-table-column label="追溯编号"  align="center">
          <template slot-scope="scope">{{scope.row.initiation}}</template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="120" align="center">
          <template slot-scope="scope" v-if="permission.ppl_order_destroy">
            <el-button size="mini"
                       type="text"
...
<script>
  ...
  import {recoveryOrder,getGameHoldOrderList,carouselDelete,free_lockDelete,updateGameRewardConfig,getGameTypeList} from '@/api/ppl/marketing';
    ...
    methods: {
     ...
     handlePublishStatusChange(index,row){
      ...
      getList() {
        ...
        getGameHoldOrderList(this.listQuery).then(response => {
